...
myftw(char *pathname, Myfunc *func) {
    ...
    if (pathlen <= strlen(pathname)) {
        ...
        if ((fullpath = realloc(fullpath, pathlen)) == NULL) {
            err_sys("realloc failed: %s", strerror(errno));
        }
    }
...
dopath(Myfunc *func) {
    ...
    if ((dp = opendir(fullpath)) == NULL) /* can't read direcotry */
    ...
    fullpath[n - 1] = 0; /* erase everything from salsh onward */
    ...
    switch (type) {
        case FTW_F:
            switch (statptr->st_mode & S_IFMT) {
                ...
                case S_IFDIR: /* direcotries should have type = FTW_D */
